...
public sealed class BackendProcessManager : IDisposable
{
    ...
    internal BackendProcessManager(
        ...
        Func<CancellationToken, Task<bool>>? apiReachabilityProbe,
        Action<ProcessStartInfo>? processStartObserver)
    {
        ...
        _executablePath = executablePath ?? Path.Combine(baseDir, "backend", exeName);
    }
    ...
    /// <summary>
    /// Returns <c>true</c> when another <c>SharpClaw.Runtime.Host</c>
    ...
    {
        if (_processOnPortProbe is not null)
            return _processOnPortProbe();
