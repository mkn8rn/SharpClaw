...
public class ModelsController(InternalApiClient api) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(CreateModelRequest request, CancellationToken ct)
    {
        try
        {
            var result = await api.PostAsync<CreateModelRequest, ModelResponse>(
                "/models", request, ct);
            return Ok(result);
        }
        catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.BadRequest)
        {
            return BadRequest(new { error = "Invalid model request." });
        }
        catch (HttpRequestException)
        {
            return StatusCode(StatusCodes.Status502BadGateway, new { error = "Internal service unavailable." });
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, UpdateModelRequest request, CancellationToken ct)
    {
        try
        {
            var result = await api.PutAsync<UpdateModelRequest, ModelResponse>(
                $"/models/{id}", request, ct);
            return result is not null ? Ok(result) : NotFound();
        }
        catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
        {
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken ct)
    {
        try
        {
            var success = await api.DeleteAsync($"/models/{id}", ct);
