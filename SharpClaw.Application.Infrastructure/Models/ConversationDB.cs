...
/// title, chat history, and permission grants.
/// <para>
/// When the conversation belongs to a context, the context's permission
/// grants act as defaults — they are used unless overridden by a
/// per-conversation grant for the same action type.  When the conversation
/// is standalone (<see cref="AgentContextId"/> is <c>null</c>), only its
/// own grants apply.
/// </para>
/// </summary>
public class ConversationDB : BaseEntity
{
    ...
    /// context's permission grants act as defaults for this conversation.
    /// </summary>
    ...
    public ICollection<ConversationPermissionGrantDB> PermissionGrants { get; set; } = [];
}
